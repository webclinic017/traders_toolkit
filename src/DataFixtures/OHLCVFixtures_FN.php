...
class OHLCVFixtures_FN extends Fixture implements FixtureGroupInterface
{
    ...
    private function importFiles($suffix, $output) 
    {
        ...
        // }
        // exit();
        $finder->in(self::DIRECTORY)->files()->name('/^[F-N][A-Z]*'.$suffix.'/')->sortByName();
        $fileCount = $finder->count();
        $output->writeln(sprintf('Found %d files for letters F-N ending in %s', $fileCount, $suffix));
