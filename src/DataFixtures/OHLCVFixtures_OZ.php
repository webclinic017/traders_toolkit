...
class OHLCVFixtures_OZ extends Fixture implements FixtureGroupInterface
{
    ...
    private function importFiles($suffix, $output) 
    {
        ...
        // }
        // exit();
        $finder->in(self::DIRECTORY)->files()->name('/^[O-Z][A-Z]*'.$suffix.'/')->sortByName();
        $fileCount = $finder->count();
        $output->writeln(sprintf('Found %d files for letters O-Z ending in %s', $fileCount, $suffix));
