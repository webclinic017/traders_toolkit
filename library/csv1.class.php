...
class CSV {
	...
	*/
	public function __construct ( $args = array( 'symbol' => 'SPY', 'start_date' => '1-Jan-2002', 'path' => '../assets/' ) ) { 
		...
		$pointer = ftell( $this->file['exist_csv'] ); // pointer must be at the "\n" character of the first quote string in the file that contains p information.
		//var_dump( $data[0], $pointer ); exit();
		...
		if ( $this->file['refresh'] ) {
			...
			if ( $this->is_trading_day( $this->quote['current_date_ts'] ) && ( $timestamp - $this->quote['current_date_ts'] ) > 9.5 * 3600  && ( $timestamp - $this->quote['current_date_ts'] ) < 23 * 3600 ) {
				///get current quote
				...
				//exit();
				//* open price */
				$sx = strpos( $quote_page, 'Open:' );
				$ex = strpos($quote_page, 'Bid:', $sx);
				$length = $ex - $sx;
				$this->quote['open'] = (float) trim( substr( $quote_page, $sx + 5, $length - 5 ) );
				//* closing price */
				if ( $sx = strpos( $quote_page, 'Last Trade:' ) ) {
					$ex = strpos( $quote_page, 'Trade Time:', $sx );
					$length = $ex - $sx;
					$this->quote['close'] = (float) trim( substr( $quote_page, $sx + 11, $length - 11 ) );
				} elseif ( $sx = strpos( $quote_page, "({$args['symbol']}) -" ) ) {
					$sx += 6;
					$ex = strpos( $quote_page, '(', $sx );
					$length = $ex - $sx;
					$temp = trim( substr( $quote_page, $sx, $length ) );
					$sx = strpos( $temp, ' ' );
					$ex = strpos( $temp, '.', $sx + 2 );
					$ex += 3; 
					$length = $ex - $sx;
					$this->quote['close'] = trim( substr( $temp, $sx, $length ) );
				}
				//* high and low prices */
				$sx = strpos( $quote_page, 'Range:' );
				$ex = strpos( $quote_page, '52wk Range:', $sx );
				$length = $ex - $sx;
				list( $low, $high ) = explode( '-', substr( $quote_page, $sx + 6, $length - 6) );

				$this->quote['low'] = (float) $low;
				$this->quote['high'] = (float) $high;
				//* volume */
				$sx = strpos( $quote_page, 'Volume:' );
				$ex = strpos( $quote_page, 'Avg Vol', $sx );
				$length = $ex - $sx;
				$volume_daily = substr( $quote_page, $sx + 7, $length - 7 );
				$this->quote['volume'] = (int) str_replace( ',','', $volume_daily );
