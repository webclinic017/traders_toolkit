...
* @class LIN_REG_SLOPE
* 
* @brief Calculates linear regression values, finds linear regression slope for a given vanule of 'n'. The class also calculates difference in slopes. The s-table is built by applying a filter for change in slope in linear regression lines from positive to negative for bearish conditions and from negative to positive for bullish. Resultant slopes of lin. reg. lines are noted and filed in the s-table. This is the only difference of this class from the lin_reg_slope3.class. That class files diffeerence in slopes as independent variable, and this class resultant slope after the slope changes sign.
*
	...
	function __construct( $symbol_name, $study_name, $study_data, $csv ) {
		...
		*/ 
		...
		for ( $indicator_data['bullish']['m'] = 0, $indicator_data['bearish']['m'] = 0; $indicator_data['bullish']['buffer'] = each( $study_data['bullish'] ), $indicator_data['bearish']['buffer'] = each( $study_data['bearish'] ); ) {
			...
			for ( $indicator_data['bullish']['n'] = 0, $indicator_data['bearish']['n'] = 0; $indicator_data['bullish']['buffer1'] = each( $indicator_data['bullish']['buffer']['value'] ), $indicator_data['bearish']['buffer1'] = each( $indicator_data['bearish']['buffer']['value'] ); ) {
				...
				//exit();
				$this->s_table_header['bullish']['name'] = sprintf( '%s_%s_s-table2_bullish%02u-%02u_c%03u_d%02u_m1%.2f_m2%.2f', $symbol_name, $study_name, $indicator_data['bullish']['m'], $indicator_data['bullish']['n'], $study_data['conf_level'], $study_data['holding_period'], $study_data['multiple'], $study_data['multiple2'] ); /** < Used to put name inside s-table in cell A1 when using Calc::dump_CSV() */
				$this->s_table_header['bearish']['name'] = sprintf( '%s_%s_s-table2_bearish%02u-%02u_c%03u_d%02u_m1%.2f_m2%.2f', $symbol_name, $study_name, $indicator_data['bearish']['m'], $indicator_data['bearish']['n'], $study_data['conf_level'], $study_data['holding_period'], $study_data['multiple'], $study_data['multiple2'] ); /** < Used to put name inside s-table in cell A1 when using Calc::dump_CSV() */
				//var_dump( $name ); exit();
		...
		*/
		for ( $key = $start; $key < $end; $key++ ) {
			...
			if ( $slope['bullish'][$key] <= 0 && $slope['bullish'][$key + 1] > 0 ) {
				$symbol_lin_reg['bullish'][$key + 1] = $slope['bullish'][$key + 1];
			}
			if ( $slope['bearish'][$key] >= 0 && $slope['bearish'][$key + 1] < 0 ) {
				$symbol_lin_reg['bearish'][$key + 1] = $slope['bearish'][$key + 1];
			}
		...
		*/
		if ( $study_data['full_study'] ) {
			...
			$symbol_series[1] = array( 'values' => $symbol_lin_reg['bullish'], 'shape' => 'circle', 'position' => $study['low'], 'pos_offset' => -20, 'show_values' => TRUE, 'transparency' => 50, 'precision' => 1, );
			$symbol_series[2] = array( 'values' => $symbol_lin_reg['bearish'], 'shape' => 'circle', 'position' => $study['high'], 'pos_offset' => 20, 'show_values' => TRUE, 'transparency' => 50, 'precision' => 1, );
			...
			*/
			...
			$annotation = sprintf( 'Liner Regression Slope (LRS) Study, Symbol: %s, Linear regression line, bars: Bullish=%s Bearish=%s, Current value of LRS: Bullish=%.2f Bearish=%.2f, Holding Per.=%02ud, Conf. Int.=%03u%%', $symbol_name, $indicator_data['bullish']['n'], $indicator_data['bearish']['n'], $slope_diff['bullish'][$end], $slope_diff['bearish'][$end], $study_data['holding_period'], $study_data['conf_level'] );
		...
		*/
		...
		///figure ymin and ymax for the slope series. This series will use secondary y-axis
		// $ymax = array();
		// $ymin = array();
		// $temp = array();
		// $ymax = max( $ymax );
		...
		$line_series = array();
		$line_series[1] = array( 'series' => $slope_diff['bullish'], 'x_axis' => 0, 'y_axis' => 0, 'color' => 'blue', 'style' => 'dash', );
		$line_series[2] = array( 'series' => $slope_diff['bearish'], 'x_axis' => 0, 'y_axis' => 0, 'color' => 'yellow', 'style' => 'dash', );
		$y_zero_line = array( $xmin => 0, $xmax => 0, );
		$line_series[3] = array( 'series' => $y_zero_line, 'x_axis' => 0, 'y_axis' => 0, 'color' => 'blue', );
		$line_series[4] = array( 'series' => $slope['bullish'], 'color' => 'blue', );
		$line_series[5] = array( 'series' => $slope['bearish'], 'color' => 'yellow', );
		//$line_series[6] = array( 'series' => $y_zero_line, 'x_axis' => 0, 'y_axis' => 0, 'color' => 'blue', 'style' => 'dash', );
		foreach( $line_series as $line ) {
			$chart->add_line_series( $line );
		}
		...
		*/
		...
		$this->output .= sprintf( '%s%s_%s_chart1.png', $study_data['study_path'], $symbol_name, $study_name );
			...
			switch ( $side ) {
				...
				case 'bullish':
					...
					if ( $slope[$key] <= 0 && $slope[$key + 1] > 0 ) {
						$rows_population[$key + 1] = ceil( $slope[$key + 1] / $study_data['multiple'] ) * $study_data['multiple'];
				...
				case 'bearish':
					$counter = FALSE; 
					if ( $slope[$key] > 0 && $slope[$key + 1] <= 0 ) {
						$rows_population[$key + 1] = floor( $slope[$key + 1] / $study_data['multiple'] ) * $study_data['multiple'];
						if ( $ext_min[$key + 1] >= 0 ) {
							$columns_population['ext_min'][$key + 1] = ceil( $ext_min[$key + 1] / $study_data['multiple2'] ) * $study_data['multiple2'];
						} else {
							$columns_population['ext_min'][$key + 1] = floor( $ext_min[$key + 1] / $study_data['multiple2'] ) * $study_data['multiple2'];
