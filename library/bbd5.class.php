...
class BBD {
	...
	function __construct( $symbol_name, $study_name, $study_data, $csv ) {
		...
		*/
		...
		$bbd_diff['bullish'] = Calc::lookup_slope( array( 'values' => $bbd['bullish'], 'bars' => 2, 'multiplier' => 1, ) );
		...
		$bbd_diff['bearish'] = Calc::lookup_slope( array( 'values' => $bbd['bearish'], 'bars' => 2, 'multiplier' => 1, ) );
		...
		for ( $key = $start; $key < $end; $key++ ) {
			...
			if ( isset( $bbd['bullish'][$key] ) ) {
				if ( $bbd['bullish'][$key] <= 0 && $bbd['bullish'][$key + 1] > 0 ) {
					...
					$symbol_bbd['bullish'][$key + 1] = $bbd_diff['bullish'][$key + 1];
				}
			}
			if ( isset( $bbd['bearish'][$key] ) ) {
				if ( $bbd['bearish'][$key] >= 0 && $bbd['bearish'][$key + 1] < 0 ) {
					...
					$symbol_bbd['bearish'][$key + 1] = $bbd_diff['bearish'][$key + 1];
				}
			}
		}
			...
			*/
			...
			foreach ( $symbol_bbd['bearish'] as $key => $value ) {
				...
				if ( array_key_exists( (string) $row, $this->s_table['bearish'] ) ) { 
					...
					//echo 'close, ext_min_avg, s_table';
					//var_dump( $study['close'][$key], $ext_min_avg, $this->s_table['bearish'][(string) $row]['ext_min_avg'] );
					$temp[$sx] = $ext_min_avg;
					$temp[$ex] = $ext_min_avg;
			...
			$annotation = sprintf( 'Bulls to Bears delta (BBd), Symbol: %s, BBd tuning: Bullish=%s Bearish=%s, Holding Per.=%02ud, Conf. Int.=%03u%%, m1=%.2f, m2=%.2f, csv_update=%s', $symbol_name, $bbd_data['bullish']['shortname'], $bbd_data['bearish']['shortname'], $study_data['holding_period'], $study_data['conf_level'], $study_data['multiple'], $study_data['multiple2'], date( 'md/H:i T', $csv->file['stat']['mtime'] ) );
		...
		*/
		...
		$line_series[3] = array( 'series' => $bbd_diff['bullish'], 'color' => 'green', 'style' => 'dash', );
		$line_series[4] = array( 'series' => $bbd_diff['bearish'], 'color' => 'red', 'style' => 'dash', );
		foreach( $line_series as $line ) {
			$chart->add_line_series( $line );
		}
	...
	private function create_s_table( $csv, $bbd_data, $study_data, $side ) {
		...
		*/
		...
		$bbd_diff = Calc::lookup_slope( array( 'values' => $bbd, 'bars' => 2, 'multiplier' => 1, ) );
		$arr_keys = array_keys( $bbd_diff );
		$first_key['bbd_diff'] = $arr_keys[0];
		//var_dump( $bbd ); exit();
		...
		*/
		...
		$bbd_diff = array_slice( $bbd_diff, $start - $first_key['bbd_diff'], -$study_data['holding_period'], TRUE );
			...
			switch ( $side ) {
				...
				case 'bullish':
					...
					if ( $bbd[$key] <= 0 && $bbd[$key + 1] > 0 ) {
						$rows_population[$key + 1] = ceil( $bbd_diff[$key + 1] / $study_data['multiple'] ) * $study_data['multiple'];
				...
				case 'bearish':
					$counter = FALSE; 
					if ( $bbd[$key] > 0 && $bbd[$key + 1] <= 0 ) {
						$rows_population[$key + 1] = floor( $bbd_diff[$key + 1] / $study_data['multiple'] ) * $study_data['multiple'];
						if ( $rows_population[$key + 1] == 0 ) $rows_population[$key + 1] = 0;
						if ( $ext_min[$key + 1] >= 0 ) {
							$columns_population['ext_min'][$key + 1] = ceil( $ext_min[$key + 1] / $study_data['multiple2'] ) * $study_data['multiple2'];
						} else {
							$columns_population['ext_min'][$key + 1] = floor( $ext_min[$key + 1] / $study_data['multiple2'] ) * $study_data['multiple2'];
