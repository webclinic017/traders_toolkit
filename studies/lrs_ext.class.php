...
class LRS_EXT {
	...
	function __construct( $symbol_name, $study_name, $study_data, $csv ) {
		...
		for ( $indicator_data['bullish']['m'] = 0, $indicator_data['bearish']['m'] = 0; $indicator_data['bullish']['buffer'] = each( $study_data['bullish'] ), $indicator_data['bearish']['buffer'] = each( $study_data['bearish'] ); ) {
			...
			for ( $indicator_data['bullish']['n'] = 0, $indicator_data['bearish']['n'] = 0; $indicator_data['bullish']['buffer1'] = each( $indicator_data['bullish']['buffer']['value'] ), $indicator_data['bearish']['buffer1'] = each( $indicator_data['bearish']['buffer']['value'] ); ) {
				...
				*/
				...
				$study_data['lrs_ext_std_dev']['bullish'] = $line['value']['lrs_ext_std_dev'];
				reset( $this->s_table['bullish'] );
				//var_dump( $study_data['lrs_ext_std_dev']['bullish'] ); exit();
				...
				*/
				...
				$study_data['lrs_ext_std_dev']['bearish'] = $line['value']['lrs_ext_std_dev'];
		...
		*/
		...
		for ( $key = $start; $key < $end; $key++ ) {
			...
			if ( isset( $slope_diff['bullish'][$key] ) ) {
				if ( $slope_diff['bullish'][$key] < 0 && $slope_diff['bullish'][$key + 1] > 0 && $slope['bullish'][$key] < ( -1 ) * $study_data['lrs_ext_std_dev']['bullish'] ) {
			...
			if ( isset( $slope_diff['bearish'][$key] ) ) {
				if ( $slope_diff['bearish'][$key] > 0 && $slope_diff['bearish'][$key + 1] < 0 && $slope['bearish'][$key] > ( 1 ) * $study_data['lrs_ext_std_dev']['bearish'] ) {
		...
		*/
		if ( $study_data['full_study'] ) {
			...
			*/
			...
			$annotation = sprintf( 'Liner Regression Slope (LRS) Study, Symbol: %s, Linear regression line, bars: Bullish=%s Bearish=%s, Holding Per.=%02ud, Conf. Int.=%03u%%, m1=%.2f, m2=%.2f', $symbol_name, $indicator_data['bullish']['n'], $indicator_data['bearish']['n'], $study_data['holding_period'], $study_data['conf_level'], $study_data['multiple'], $study_data['multiple2'] );
		...
		*/
		...
		$horiz_line = array( $xmin => -$study_data['lrs_ext_std_dev']['bullish'], $xmax => -$study_data['lrs_ext_std_dev']['bullish'], );
		$line_series[5] = array( 'series' => $horiz_line, 'color' => 'blue', ); 
		$horiz_line = array( $xmin => $study_data['lrs_ext_std_dev']['bearish'], $xmax => $study_data['lrs_ext_std_dev']['bearish'], );
